...
cc-sdd をフォークし、**code-review-graph (CRG)** による仕様↔コードのトレーサビリティを統合したバージョン。`@impl` タグによる要件↔コードの自動追跡、影響範囲分析、仕様ドリフト検出を提供します。
...
### CRG 対応（15スキル）
ほぼ全てのスキルが code-review-graph と連携し、コードグラフを活用した分析・検証を実行します:

| スキル | CRG 連携 |
|--------|---------|
| `kiro-discovery` | 既存拡張時に影響範囲を自動表示 |
| `kiro-spec-design` | コードグラフ分析でアーキテクチャ設計を強化 |
...
| `kiro-review` | CRG-enhanced boundary respect |
...
- **pre-commit hook**: `setup-crg.sh` がコミット時のスナップショット自動更新をセットアップ
- **CI/CD ゲート**: `scripts/check_drift.py --diff --gate` で仕様ドリフトを自動検出
...
- [Windows セットアップスクリプト](./scripts/quickstart.ps1)
